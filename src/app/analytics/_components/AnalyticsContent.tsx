...
// 'use client' does NOT prevent SSR — Next.js still renders client components
// on the server once for hydration. So we must explicitly guard with typeof window
// to ensure fetch() (with a relative URL) only runs in the browser.
//
// The variable is module-scoped so it survives Suspense unmount/remount cycles
...
export default function AnalyticsContent() {
  ...
  // use() suspends until the browser-side promise resolves.
  const applications = use(getApplicationsPromise());
  ...
  const activeApps    = totalApps - savedApps;
  ...
  const kpis = [
    ...
    { label: 'Interview Rate', value: `${interviewRate}%`, icon: <Users  size={16} />, color: '#A78BFA', desc: `${interviewingApps} interviews booked` },
    { label: 'Offer Rate',     value: `${offerRate}%`,     icon: <Target size={16} />, color: '#34D399', desc: `${offerApps} offers received` },
    { label: 'Avg Match',      value: `${avgMatchScore}%`, icon: <Zap    size={16} />, color: '#FBBF24', desc: `Across ${matchApps.length} analyzed roles` },
  ];
  ...
  if (filteredApps.length > 0) {
    ...
    const maxDate = Math.max(...dates, minDate + 1000);
  ...
  // Funnel
  const FUNNEL = [
    { label: 'Discovered / Saved', value: totalApps,                    pct: totalApps > 0 ? 100 : 0,                                                              color: '#60A5FA' },
    { label: 'Applied / Active',   value: activeApps,                   pct: totalApps > 0 ? Math.round((activeApps                      / totalApps) * 100) : 0,  color: '#A78BFA' },
    { label: 'Interviews Booked',  value: interviewingApps + offerApps, pct: totalApps > 0 ? Math.round(((interviewingApps + offerApps)  / totalApps) * 100) : 0,  color: '#FBBF24' },
    { label: 'Offers Received',    value: offerApps,                    pct: totalApps > 0 ? Math.round((offerApps                       / totalApps) * 100) : 0,  color: '#34D399' },
  ];
  ...
  filteredApps.forEach(app => {
    if (Array.isArray(app.missingSkills)) {
      app.missingSkills.forEach(skill => {
        const s = skill.trim();
        if (s) skillsFreq[s] = (skillsFreq[s] || 0) + 1;
      });
    }
  });
  ...
  // Sources
  const sourcesFreq: Record<string, number> = { LinkedIn: 0, Indeed: 0, Naukri: 0, Direct: 0 };
  filteredApps.forEach(app => {
    const url = (app.jobUrl || '').toLowerCase();
    if      (url.includes('linkedin.com')) sourcesFreq.LinkedIn++;
    else if (url.includes('indeed.com'))   sourcesFreq.Indeed++;
    else if (url.includes('naukri.com'))   sourcesFreq.Naukri++;
    else                                   sourcesFreq.Direct++;
  });
              ...
              <>
                <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: '160px', overflow: 'visible' }}>
                  ...
                  {[0.25, 0.5, 0.75, 1].map((f) => (
                    <line key={f} x1={PAD} y1={toY(maxVal * f)} x2={W - PAD} y2={toY(maxVal * f)} stroke="#1A1A1A" strokeWidth="1" />
                  ))}
                  ...
                  {points.slice(-3).map((v, i) => (
                    <circle key={i} cx={toX(points.length - 3 + i)} cy={toY(v)} r="4" fill="#60A5FA" stroke="#111" strokeWidth="2" />
                  ))}
                </svg>
