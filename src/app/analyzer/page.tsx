...
const HAS_GEMINI_KEY = !!process.env.NEXT_PUBLIC_GEMINI_API_KEY;
...
export default function AnalyzerPage() {
  ...
  const handleAnalyze = async () => {
    ...
    setResult(null);
    try {
      const data = await analyzeResumeDirect({ resumeText: fileText, jobDescription });
      setResult(data);
      ...
      try {
        ...
        const fallback = await local({ resumeText: fileText, jobDescription });
            ...
            {!HAS_GEMINI_KEY && (
              <div style={{ padding: '14px 16px', background: 'rgba(96,165,250,0.06)', border: '1px solid rgba(96,165,250,0.2)', borderRadius: '11px', display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: '12px', color: 'var(--text-secondary)', lineHeight: 1.6 }}>
                  <span style={{ color: '#60A5FA', fontWeight: 700 }}>Free Gemini AI available.</span> Add <code style={{ background: 'rgba(96,165,250,0.1)', padding: '1px 5px', borderRadius: '4px' }}>NEXT_PUBLIC_GEMINI_API_KEY</code> to your <code style={{ background: 'rgba(96,165,250,0.1)', padding: '1px 5px', borderRadius: '4px' }}>.env.local</code> for 10× deeper analysis. Get a free key at <span style={{ color: '#60A5FA' }}>aistudio.google.com</span>.
                </div>
              </div>
            )}
          </div>
